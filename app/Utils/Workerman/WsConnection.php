...
use App\{
    CurrencyMatch, CurrencyQuotation, Logic\MicroTradeLogic, MarketHour, UserChat
};
...
//    protected $server_address = 'ws://api.huobi.pro:443/ws';
    ...
    protected $server_time_out = 5; //服务器响应超时
    ...
    public function onConnect($con)
    {
        ...
        $filename = 'onConnect'.date('Ymd') . '.log';
        file_exists($path) || @mkdir($path);
        error_log(date('Y-m-d H:i:s') . ' ' . $this->server_address . 'onConnect' . PHP_EOL, 3, $path . $filename);
    ...
    protected function onMarketKline($con, $data, $match)
    {
        ...
        EsearchMarket::dispatch($market_data)->onQueue('esearchMarket');
    ...
    public function sendDepthData()
    {
        ...
        foreach ($market_depth as $depth_data) {
            //SendMarket::dispatch($depth_data);
            UserChat::sendText($depth_data);
        }
    }
    ...
    public function sendMatchTradeData()
    {
        ...
        foreach ($market_trade as $trade_data) {
           // SendMarket::dispatch($trade_data);
            UserChat::sendText($trade_data);
...
//            echo "\t" . date('Y-m-d H:i:s') . ' 发送' . $key . ',价格:' . $kline_data['close'] . PHP_EOL;
            ...
            if (bc_comp($kline_data['close'], 0) > 0) {
                LeverUpdate::dispatch($params);
                CoinTradeHandel::dispatch($params);
...
//                echo '秒合约时间:' . time() . ', Symbol:' . $key . '.' . $period . '数据' . PHP_EOL;
                if ($period == '1min') {
                    ...
                    // if($c_m->open_microtrade == 1){
                       // HandleMicroTrade::dispatch($symbol['kline_data']);
    ...
    {
        $path = base_path() . '/storage/logs/wss/';
        $filename = 'writeMarketKline'.date('Ymd') . '.log';
        file_exists($path) || @mkdir($path);
        error_log(date('Y-m-d H:i:s') . ' ' . $this->server_address . 'writeMarketKline' . PHP_EOL, 3, $path . $filename);
        if ($this->worker_id < 8) {
            $market_data = self::$marketKlineData;
            foreach ($market_data as $period => $data) {
                foreach ($data as $key => $symbol) {
//                    echo '处理' . $key . '.' . $period . '数据' . PHP_EOL;
                    ...
                    //SendMarket::dispatch($symbol['kline_data']);
                    UserChat::sendText($symbol['kline_data']);
                    EsearchMarket::dispatch($symbol['market_data']);//统一用一个队列
                    if ($period == '1min') {
                        // var_dump($symbol['kline_data']);
                        //推送一分钟行情
                        //SendMarket::dispatch($symbol['kline_data'])->onQueue('kline.1min');
                        //更新币种价格
                        UpdateCurrencyPrice::dispatch($symbol['kline_data']);
                    } elseif ($period == '1day') {
                        //推送一天行情
