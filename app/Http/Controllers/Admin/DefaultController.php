...
class DefaultController extends Controller
{
    ...
    public function trackOrder()
    {
        $where = ['status' => 1];
        //充值数据
        $recharge = Db::table('charge_req')->where($where)->count();
        //提现数据
        $withdraw = Db::table('users_wallet_out')->where($where)->count();
        
        $data = ['code' => 200, 'msg' => 'Request successful.', 'data' => ['recharge' => $recharge, 'withdraw' => $withdraw]];
