...
class AccountController extends Controller
{
    ...
    public function chargeMentionMoney2(Request $request)
    {
    ...
    public function chargeMentionMoney(Request $request){
        ...
        $userWalletOutList = $userWalletOut->where('user_id',$user_id)->orderBy('id', 'desc')->paginate($limit);
        return $this->success($userWalletOutList);
    }
    public function chargeMentionMoney3(Request $request){

        $limit = $request->get('limit', 5);
        $user_id = Users::getUserId();
        $charge_req =  \DB::table('charge_req');
        $userWalletOutList = $charge_req->where('uid',$user_id)->orderBy('id', 'desc')->paginate($limit);
        return $this->success($userWalletOutList);
    }
}
