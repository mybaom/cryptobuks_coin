...
class OfferProductController extends Controller
{
    ...
    public function getNewTimeData()
    {
        ...
        try {
            ...
            // 涨幅百分比
            $proportion = substr($this->numberAddSubRand(round(abs(($currentData['open_price'] - $yesteDayData['open_price']) / $yesteDayData['open_price']) * 100, 2), $nowPrice > $yesteDayData['open_price']), 0, 5);
            $price1 = $this->subPrice($this->numberAddSubRand($this->subPrice( rand($currentData['lowest_price'] * 10000000000, $currentData['highest_price'] * 10000000000) / 10000000000), $nowPrice > $yesteDayData['open_price']));
            $price3 = $this->subPrice($this->numberAddSubRand($this->subPrice(rand($currentData['lowest_price'] * 10000000000, $currentData['highest_price'] * 10000000000) / 10000000000), $nowPrice > $yesteDayData['open_price']));
