...
//        $user_id = Users::getUserId();
    ...
    public function info(Request $request)
    {
        ...
        $user['customer_service'] = Setting::getValueByKey("customer_service");
        $user['appxiazai'] = Setting::getValueByKey("appxiazai");
...
//        $legal_wallet['CNY'] = '';
    ...
    {
        $service = Setting::getValueByKey("customer_service");
    ...
    public function getMyBankList()
    {
        ...
        $list = MyBank::where('user_id', $user_id)->where('type', $type)->paginate($limit);
        return $this->success(array(
            "user_id" => $user_id,
            "data" => $list->items(),
            "limit" => $limit,
    ...
    public function residence(){
        $user_id = Users::getUserId();
        $name = Input::get("name", "");//居住地
        $front_pic = Input::get("front_pic", "");//居住地照片
        if (empty($name) ||empty($front_pic)) {
        ...
        $userreal_number = UserJuzu::where("address", $name)->count();
        if ($userreal_number > 0) {
            return $this->error("该居住地址已认证过!");
        }
        ...
        try {
            ...
            $userreal = new UserJuzu();

            $userreal->user_id = $user_id;
            $userreal->address = $name;
...
//        var_dump($userreal_number);die;
    ...
    public function userCenter()
    {
        ...
        if (empty($userjuzu)) {
            ...
            $user['juzu_status'] = $userjuzu['review_status'];
        }
        ...
        try {
            ...
            foreach ($user_wallets as $user_wallet) {
                ...
                if ($currency->type == 'eth') {
                    ...
                    $url = 'http://154.92.18.240/wallet/btc/balance?address=' . $address;
                    ...
                    // echo $address;
                    $url = 'http://154.92.18.240/wallet/usdt/balance?address=' . $address;
                    $content = RPC::curl($url, false, 0, 0);
                    $content = @json_decode($content, true);
                    if (isset($content["code"]) && $content["code"] == 0) {
                        $content["result"] = $content['data']['balance'];
