    ...
    public function handle(){
        ...
        foreach ($products as $k => $v) {
            ...
            while ($getData) {
                ...
                if(
                    $this->convert_scientific_number_to_normal(abs($data['highest_price']-$data['lowest_price'])/$data['highest_price']) < 0.005
                    &&
                    $this->convert_scientific_number_to_normal(abs($data['close_price']-$data['open_price'])/$data['close_price']) < 0.005
                ){
                    $getData = false;
                }
            }
