...
class LeverTransaction extends Model
{
    ...
    public static function newPrice($legal_id, $currency_id, $price, $timestamp = null)
    {
        ...
        $end = microtime(true);
        LeverHandle::dispatch($params)->onConnection('sync');
       ...
       // echo '止盈止亏消耗:' . ($end - $start) . '秒' . PHP_EOL;
        ...
        if (count($levers) > 0) {
            foreach ($levers as $trade) {
                //推送风险率和订单
                LeverPushTrade::dispatch($trade->user_id, $legal_id, $currency_id);
...
//            }
            ...
            DB::commit();
            LeverClose::dispatch($affect_result, false);
            return true;
        } catch (\Exception $ex) {
            DB::rollBack();
    ...
    public static function checkNeedStopPriceTrade($user_id = 0, $legal_id = 0, $currency_id = 0)
    {
        ...
        DB::commit();
        $result > 0 && LeverClose::dispatch($task_list, true);
    }
